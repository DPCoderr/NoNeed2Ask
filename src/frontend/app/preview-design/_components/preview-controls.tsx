...
export function PreviewControls<T extends string>({ pageName, scenarios, scenario, onScenarioChange, notice, onDismiss }: {
  ...
  return (
    <aside aria-label="Design preview controls" className="mx-auto flex w-full max-w-[1320px] flex-wrap items-center gap-3 px-5 pb-8 text-xs text-slate-500 sm:px-8 xl:px-10">
      ...
      <select aria-label="Preview page" value={pageName} onChange={(event) => router.push(`/preview-design/${event.target.value}`)} className={selectClass}><option value="dashboard">Dashboard</option><option value="applications">Applications</option><option value="public-page">Public page</option></select>
